...
#   - ✅ MP3 dönüştürme UX iyileştirmesi - postprocessor_hook (v2.9)
#   - ✅ analiz_thread çift except bug düzeltmesi (v2.9)
#   - ✅ lambda closure bug düzeltmesi (v2.9)
...
class YSVideoDownloader:
    def __init__(self, root):
        ...
        self.mp3_donusturme_bildirildi = False
    ...
    def arayuz_olustur(self):
        ...
        self.download_button = tk.Button(
            ...
            height=2, cursor="" if IS_MACOS else "hand2",
            **buton_disabled_renk()
        )
    ...
    def analiz_thread(self, url):
        ...
        try:
            ...
            self.root.after(0, lambda msg=hata_mesaji: self.analiz_hatali(msg))
    ...
    def postprocessor_hook(self, d):
        """MP3 dönüştürme durumunu takip eder (Thread-safe)"""
        if d['status'] == 'started':
            pp_name = d.get('postprocessor', '')
            if ('Audio' in pp_name or 'Extract' in pp_name) and not self.mp3_donusturme_bildirildi:
                self.mp3_donusturme_bildirildi = True
                self.root.after(0, self._mp3_donusturme_basladi)

        elif d['status'] == 'finished':
            pp_name = d.get('postprocessor', '')
            if 'Audio' in pp_name or 'Extract' in pp_name:
                self.root.after(0, self._mp3_donusturme_bitti)

    def _mp3_donusturme_basladi(self):
        """MP3 dönüştürme başladığında UI'yi günceller"""
        self.log_yaz("🎵 MP3'e dönüştürülüyor... (Bu işlem biraz sürebilir)")
        self.download_button.config(text="🎵 MP3'E DÖNÜŞTÜRÜLÜYOR...", **buton_renk("#e67e22"))
        self.progress.config(mode='indeterminate')
        self.progress.start(PROGRESS_ANIMATION_SPEED)

    def _mp3_donusturme_bitti(self):
        """MP3 dönüştürme bittiğinde UI'yi normale döndürür"""
        self.progress.stop()
        self.progress.config(mode='determinate')
        self.progress['value'] = 100
        self.log_yaz("✅ Dönüştürme tamamlandı.")
    ...
    def ytdlp_versiyon_kontrol(self):
        ...
        try:
            ...
            # SSL context - macOS bundle'da sertifika sorunu olabileceğinden
